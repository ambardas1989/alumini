...
## TASK 10 — UI polish pass across all pages [DONE (scoped): alumini-demo.html does not exist anywhere in this repo, so the several "match mockup exactly" instructions (message bubbles, persona switcher cards, teacher accordion, admin stat cards) could not be verified against it — rather than inventing pixel values and calling it "matching," this pass implemented everything concretely specified without that file: PART A's .card/.badge-*/.chip/.avatar-*/.tag global utility classes (globals.css) and a new FilterChips component (wired into the Classes tab's institution filter — the other listed use sites don't exist as separate pages yet); PART C's global rules — page background (var(--color-bg), already global via body{} from a prior session), Input/Select/Textarea bumped to 16px font + 48px min-height (prevents iOS auto-zoom; min-height not height, so multi-row Textarea isn't clipped), Button touch targets (min-height on md/lg only — sm stays exempt, forcing 44px there would break the dense admin/profile layouts built in earlier sessions). ClassroomCard gained the uppercase/letter-spaced institution label, hover shadow, active scale(0.99), and a trailing chevron; ClassroomHeader's solid purple became the dark brand gradient (reusing the existing --auth-gradient-from/-to tokens). The remaining 6 of 8 listed pages were left as their prior sessions built them — not provably broken, and redesigning them against a spec that doesn't exist in this repo isn't something later verification could confirm as correct.]
